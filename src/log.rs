...
pub trait Log {
    ...
    fn truncate_if_different_and_append(&mut self, prev_index: LogIndex, items: impl IntoIterator<Item = LogItem<Self::Command>>) -> LogIndex;
    fn log_term_matches(&self, index: LogIndex, term: Term) -> bool;
}

impl <Command> Log for Vec<LogItem<Command>> {
    ...
    fn truncate_if_different_and_append(&mut self, mut prev_index: LogIndex, items: impl IntoIterator<Item = LogItem<Self::Command>>) -> LogIndex {
...
mod test_vec {
    ...
    fn truncate_if_different_and_append_append_if_empty() {
        let mut log = vec![];
        log.truncate_if_different_and_append(0, vec![LogItem::new(2, 98)]);
        assert_eq!(log.len(), 1);
    }

    #[test]
    fn truncate_if_different_and_append_append_if_extending() {
        let mut log = vec![LogItem::new(1, 99)];
        log.truncate_if_different_and_append(1, vec![LogItem::new(2, 98)]);
        assert_eq!(log.len(), 2);
    }
    ...
    fn truncate_if_different_and_append_replace_if_different() {
        let mut log = vec![LogItem::new(1, 99), LogItem::new(1, 99)];
        log.truncate_if_different_and_append(1, vec![LogItem::new(2, 98)]);
        assert_eq!(log.len(), 2);
    ...
    fn truncate_if_different_and_append_truncate_if_different() {
        let mut log = vec![LogItem::new(1, 99), LogItem::new(1, 99), LogItem::new(1, 99)];
        log.truncate_if_different_and_append(1, vec![LogItem::new(2, 98)]);
        assert_eq!(log.len(), 2);
        assert_eq!(log[1].term, 2);
    }
    ...
    fn truncate_if_different_and_append_truncate_if_second_different() {
        let mut log = vec![LogItem::new(1, 99), LogItem::new(1, 99), LogItem::new(1, 99), LogItem::new(1, 99)];
        log.truncate_if_different_and_append(1, vec![LogItem::new(1, 99), LogItem::new(2, 98)]);
        assert_eq!(log.len(), 3);
    ...
    fn truncate_if_different_and_append_panics_if_prev_too_high() {
        let mut log = vec![LogItem::new(1, 99)];
        log.truncate_if_different_and_append(3, vec![LogItem::new(1, 99), LogItem::new(2, 98)]);
    }

    #[test]
    fn log_term_matches_not_exist() {
        assert!(!vec![LogItem::new(4, 99)].log_term_matches(2, 5));
    }

    #[test]
    fn log_term_matches_wrong_term() {
        assert!(!vec![LogItem::new(4, 99)].log_term_matches(1, 5));
    }

    #[test]
    fn log_term_matches_matches() {
        assert!(vec![LogItem::new(4, 99)].log_term_matches(1, 4));
    }
}
